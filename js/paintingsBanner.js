...
// Function to check if elements have stopped moving
function checkMovement() {
  const tolerance = 1; // Adjust the tolerance value based on your needs
  // console.log(loop);
  const elementsStopped = loop._targets.some((item, index) => {
    let currentX = gsap.getProperty(item, "xPercent");
    // console.log(currentX);
    if (Math.abs(currentX - lastX[index]) < tolerance) {  // Check if movement is smaller than tolerance
      return true;
    }
  ...
  if (elementsStopped) {
    console.log("Detected animation stopped. Refreshing...");
    // Refresh the page with cache clearing
    window.location.reload(true);
  }
}

// // If loop doesn't have _targets, manually add them
loop._targets = gsap.utils.toArray(".image");

// // Initialize lastX with the current X positions of items
loop._targets.forEach((item, index) => {
